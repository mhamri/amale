...
test('a transient provider rate limit is retried instead of blocking the chunk',async t=>{
 ...
 process.env.AMALE_PI_ENTRY=entry;process.env.AMALE_NODE=process.execPath;
...
test('runtime resolution prefers explicit overrides, then bun, then the running executable',async t=>{const old={runtime:process.env.AMALE_RUNTIME,node:process.env.AMALE_NODE,path:process.env.PATH};t.after(()=>{for(const [name,value] of [['AMALE_RUNTIME',old.runtime],['AMALE_NODE',old.node],['PATH',old.path]])if(value===undefined)delete process.env[name!];else process.env[name!]=value;});
 delete process.env.AMALE_NODE;process.env.AMALE_RUNTIME='/explicit/runtime';assert.equal(await jsRuntime(),'/explicit/runtime');
 delete process.env.AMALE_RUNTIME;process.env.AMALE_NODE='/legacy/node';assert.equal(await jsRuntime(),'/legacy/node');
 delete process.env.AMALE_NODE;assert.match(await jsRuntime(),/bun/);
