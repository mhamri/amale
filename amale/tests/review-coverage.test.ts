...
async function fixture(t:any){const dir=await mkdtemp(join(tmpdir(),'amale-review-coverage-'));t.after(()=>rm(dir,{recursive:true,force:true}));const store=await c.start(dir,{id:'fixture',host:{kind:'codex',model:'fixture-host'},intent:'Review actual coverage',criteria:['Run result supported']});await c.plan(store,{tasks:[{id:'a',title:'a',goal:'a',phase:'one',deps:[],resources:['a'],criteria:['Task result supported'],checks:[],kind:'code'}],integrationChecks:[]});await fixtureClaim(store,'a',{workspace:dir,model:'fixture-author'});await c.result(store,'a',{synthetic:true});return {store,dir};}
