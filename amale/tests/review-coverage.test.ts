...
async function fixture(t:any){const dir=await mkdtemp(join(tmpdir(),'amale-review-coverage-'));t.after(()=>rm(dir,{recursive:true,force:true}));const store=await c.start(dir,{shape:clearCut,id:'fixture',host:{kind:'codex',model:'fixture-host'},intent:'Review actual coverage',criteria:['Run result supported']});await c.plan(store,{tasks:[{id:'a',title:'a',goal:'a',phase:'one',deps:[],resources:['a'],criteria:['Task result supported'],checks:[],kind:'code'}],integrationChecks:[]});await fixtureClaim(store,'a',{workspace:dir,model:'fixture-author'});await c.result(store,'a',{synthetic:true});return {store,dir};}
...
test('review probe registration creates check work and fresh review without consuming repair cycles',async t=>{const {store,dir}=await fixture(t);await record(store,dir,await coverage(store));const probe={id:'probe',command:process.execPath,args:['-e','process.exit(0)']};await c.addReviewCheck(store,'a',probe);let task=c.taskOf(await store.load(),'a');assert.equal(task.review,undefined);assert.equal(task.cycles,0);assert.ok(task.output);assert.equal((await c.next(store)).action,'check');await assert.rejects(()=>c.addReviewCheck(store,'a',probe),/Duplicate/);await assert.rejects(()=>c.addReviewCheck(store,'a',{...probe,id:'bad/id'}),/Invalid identifier/);await assert.rejects(()=>c.addReviewCheck(store,'a',{id:'invalid-command',command:'',args:[]}),/Commands/);await c.check(store,'a','probe');assert.equal((await c.next(store)).action,'review');await record(store,dir,await coverage(store));await c.accept(store,'a');await assert.rejects(()=>c.addReviewCheck(store,'a',{...probe,id:'later'}),/idle task/);});
