...
async function executeMain(args=process.argv.slice(2)){
 ...
 if(operation==='list'){try{return (await readdir(join(resolve(workspace),'.amale','runs'))).filter(id=>/^[a-zA-Z0-9_-]+$/.test(id));}catch(e){if((e as NodeJS.ErrnoException).code==='ENOENT')return [];throw e;}}
 ...
 case 'finish':await core.finish(store,input.claims);break;
 ...
 case 'record-decision':await store.transaction(s=>{core.invariant(input.id&&input.question&&input.answer&&input.reason&&['user','host'].includes(input.source),'ID, question, answer, source and evidence/reason required');core.invariant(!s.decisions.some(d=>d.id===input.id),'Decision ID exists');s.decisions.push({id:input.id,question:input.question,criteria:{accepted:input.answer},choice:'accepted',source:input.source,reason:input.reason,state:{},revision:s.revision});core.event(s,'recorded-decision',{id:input.id,source:input.source});});break;
