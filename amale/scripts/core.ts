...
async function priorRuns(workspace:string,exclude:string):Promise<{id:string;intent:string;criteria:string[]}[]>{
 let names:string[];try{names=await readdir(join(workspace,'.amale','runs'));}catch{return [];}
 const runs:{id:string;intent:string;criteria:string[]}[]=[];
 for(const name of names){if(name===exclude)continue;try{
  const dir=join(workspace,'.amale','runs',name),revisions=(await readdir(dir)).filter(n=>/^revision-\d{9}\.json$/.test(n)).sort();if(!revisions.length)continue;
