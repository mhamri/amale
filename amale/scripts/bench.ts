...
export async function prepare(workspace:string){
 const root=join(await realpath(workspace),'.amale','bench');
...
export async function bench(workspace:string,runId:string,host:{kind:string;model:string}){
 ...
 const store=await start(workspace,{id:runId,host,intent:'Benchmark Amale delegation: four independent utility modules, each proven by a pre-written test file',
...
export function renderScorecard(result:Awaited<ReturnType<typeof bench>>){
 ...
 return [`Amale delegation benchmark: ${result.passed}/${result.total} checks passed in ${(result.elapsedMs/1000).toFixed(1)}s`,...lines,
