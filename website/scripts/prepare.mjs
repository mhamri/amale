...
import { withTracking } from '../src/lib/tracking.ts';
...
for (const name of ['planning', 'execution', 'review', 'runtime', 'parallelism', 'recovery', 'verification', 'effort']) {
  await copyFile(new URL(`../../amaleh/references/${name}.md`, import.meta.url), new URL(`sources/${name}.md`, publicDir));
}
const workflow = await readFile(new URL('../../DESIGN.html', import.meta.url), 'utf8');
await writeFile(new URL('workflow.html', publicDir), withTracking(workflow));
