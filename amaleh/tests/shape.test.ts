...
async function run(t:any,shape?:c.ShapeInput){
 const dir=await mkdtemp(join(tmpdir(),'amaleh-shape-'));t.after(()=>rm(dir,{recursive:true,force:true}));
...
test('real options stay open until the user picks one, and the pick becomes a requirement',async t=>{
 const {store}=await run(t,{understanding:'The hero must sell Amaleh in one glance',gaps:['No headline copy was supplied'],pushback:['Three paragraphs bury the animation'],additions:['Show every flash model logo'],mentor:['A hero earns attention with one promise, not a feature list'],options,recommendation:'headline'});
 ...
 await c.shape(store,{understanding:'The hero must sell Amaleh in one glance',options,recommendation:'headline',chosen:{option:'headline',quote:'go with the headline one'}});
