...
import { piRun, resolveWrites, workerPrompt } from '../scripts/adapters.ts';
...
const escapingWorker=(dir:string,escaped:string[])=>async(store:c.Store,id:string,input:{workspace?:string})=>{
 await fixtureClaim(store,id,{workspace:input.workspace??dir,model:'deepseek/flash'});
 const reason=`Worker wrote outside the task workspace: ${escaped.join(', ')}`;
 await store.transaction(s=>{const t=c.taskOf(s,id);t.status='blocked';t.owner=undefined;t.blocked=reason;});
