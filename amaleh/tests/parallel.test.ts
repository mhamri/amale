...
async function setup(t:any,shared=false){
 const root=await mkdtemp(join(tmpdir(),'amaleh-frontier-'));t.after(()=>rm(root,{recursive:true,force:true}));
 const store=await c.start(root,{shape:clearCut,id:'fixture',host:{kind:'codex',model:'fixture-host'},intent:'Expose independent verification',criteria:['Preserve verification gates']});
 await c.plan(store,{tasks:['a','b','c'].map(id=>({id,title:id,goal:id,phase:'one',deps:[],resources:[shared?'shared':id],criteria:['pass'],kind:'code',checks:[{id:'test',command:process.execPath,args:['-e',"try{const c=require('fs').readFileSync(require('path').join(process.cwd(),'app.txt'),'utf8');process.exit(c.includes('corrected')?0:1)}catch{process.exit(1)}"],role:'probe' as const}]})),integrationChecks:[]});
 for(const id of ['a','b','c']){const dir=join(root,id);await mkdir(dir);writeFileSync(join(dir,'app.txt'),'original');await fixtureClaim(store,id,{workspace:dir,model:'fixture-author'});await c.result(store,id,{fixture:true});}
 return store;
}
test('frontier exposes independent checks and reviews beyond the focus task',async t=>{
 ...
 for(const id of ['a','b','c']){const s=await store.load(),dir=c.taskOf(s,id).workspace!;writeFileSync(join(dir,'app.txt'),'corrected');}
