...
test('resume invalidates obsolete downstream evidence',async t=>{const {dir,store}=await fixture(t);const a=join(dir,'.amaleh','a'),b=join(dir,'.amaleh','b');await mkdir(a,{recursive:true});await mkdir(b,{recursive:true});await writeFile(join(a,'amount'),'10');await c.plan(store,{tasks:[task('a'),task('b',['a'])],integrationChecks:[]});await deliver(store,a,'a');await deliver(store,b,'b');await writeFile(join(a,'amount'),'11');await c.resume(store);const state=await store.load();assert.equal(c.taskOf(state,'a').status,'ready');assert.equal(c.taskOf(state,'b').status,'ready');assert.equal(c.taskOf(state,'b').integrated,undefined);});
